...
const emp = (id: string, color = '#3b82f6'): Employee => ({
  id, name: `Name-${id}`, email: `${id}@x`, hireDate: '2024-01-01', annualSalary: 60000, role: 'r', color,
});
const proj = (id: string, eligible: boolean, color = '#9ca3af'): Project => ({
  id, name: id, description: '', isSredEligible: eligible, color,
});
